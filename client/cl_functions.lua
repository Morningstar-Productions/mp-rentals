...
local function CreatePeds()
    ...
    for i = 1, #landcoords do
        Renewed.addPed({
            model = `a_m_y_business_03`,                                      -- The ped to be spawned
            dist = 25,                                                        -- Distance of when the ped should be loaded in
            coords = vec3(landcoords[i].x, landcoords[i].y, landcoords[i].z), -- The coords for the ped
            heading = landcoords[i].w,                                        -- Heading of the ped
    ...
    for i = 1, #aircoords do
        Renewed.addPed({
            model = `s_m_y_airworker`,                                     -- The ped to be spawned
            dist = 25,                                                     -- Distance of when the ped should be loaded in
            coords = vec3(aircoords[i].x, aircoords[i].y, aircoords[i].z), -- The coords for the ped
            heading = aircoords[i].w,                                      -- Heading of the ped
            scenario = 'WORLD_HUMAN_CLIPBOARD',                            -- Use this if you want the ped to play a scenario
            freeze = true,                                                 -- use this if the ped should be frozen
            invincible = true,                                             -- use this if the ped should be invincible
            tempevents = true,                                             -- use this if the ped should block temporary events
    ...
    for i = 1, #boatcoords do
        Renewed.addPed({
            model = `mp_m_boatstaff_01`,                                      -- The ped to be spawned
            dist = 25,                                                        -- Distance of when the ped should be loaded in
            coords = vec3(boatcoords[i].x, boatcoords[i].y, boatcoords[i].z), -- The coords for the ped
            heading = boatcoords[i].w,                                        -- Heading of the ped
            scenario = 'WORLD_HUMAN_CLIPBOARD',                               -- Use this if you want the ped to play a scenario
            freeze = true,                                                    -- use this if the ped should be frozen
            invincible = true,                                                -- use this if the ped should be invincible
            tempevents = true,                                                -- use this if the ped should block temporary events
...
local function CreateRentalBlips()
    for _, blip in pairs(config.vehBlips) do
        Utils.createBlips(blip.title, vec3(blip.coords.x, blip.coords.y, blip.coords.z), blip.id, blip.scale or 0.65,
        blip.color)
    end
end
